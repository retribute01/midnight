...
contract LiquidationTest is BaseTest {
    ...
    function testLiquidateObligationUnitsInput(uint256 units, uint256 repaid, uint256 liquidationOraclePrice) public {
        ...
        assertEq(
            seizedAssets,
            repaid.mulDivDown(obligation.collaterals[0].lif, WAD)
    ...
    function testLiquidateCollateralInput(uint256 units, uint256 seized, uint256 liquidationOraclePrice) public {
        ...
        seized = bound(
            ...
            0,
            UtilsLib.min(
                units.mulDivDown(obligation.collaterals[0].lif, WAD)
                    .mulDivDown(ORACLE_PRICE_SCALE, liquidationOraclePrice),
                initialCollateral
            )
        );
        ...
        assertEq(
            repaidUnits,
            seized.mulDivUp(liquidationOraclePrice, ORACLE_PRICE_SCALE).mulDivUp(WAD, obligation.collaterals[0].lif),
            "repaid units"
        );
    ...
    function testLiquidateWithBadDebtSeizedInput(uint256 units, uint256 seized, uint256 liquidationOraclePrice) public {
        ...
        uint256 lif0 = obligation.collaterals[0].lif;
    ...
    function testLiquidatePostMaturityPartialLIF(
        ...
        uint256 lif = WAD + (obligation.collaterals[0].lif - WAD) * delay / TIME_TO_MAX_LIF;
    ...
    function testRcfThresholdEnforcesRecoveryCloseFactor(
        ...
        uint256 remainingRepayable = collatAmount.mulDivDown(liquidationOraclePrice, ORACLE_PRICE_SCALE)
            .mulDivDown(WAD, obligation.collaterals[0].lif).zeroFloorSub(maxRepaid);
    ...
    function testLiquidateFullyRepayOrFullySeizeWhenRcfDeactivated(
        ...
        uint256 repayableDebt = collateral1.mulDivDown(WAD, obligation.collaterals[0].lif)
            + collateral2.mulDivDown(WAD, obligation.collaterals[1].lif);
        ...
        uint256 collateralNeededToRepayAll = units.mulDivDown(obligation.collaterals[0].lif, WAD);
    ...
    function testRecoveryCloseFactorMultipleCollaterals(uint256 units) public {
        ...
        .mulDivUp(WAD, WAD - obligation.collaterals[liqIdx].lif.mulDivUp(obligation.collaterals[liqIdx].lltv, WAD));
    ...
    function badDebtPriceDown() internal view returns (uint256) {
        return obligation.collaterals[0].lltv * obligation.collaterals[0].lif;
    }
    ...
    function badDebtPriceUp(uint256 units) internal view returns (uint256) {
        ...
        return units.mulDivUp(obligation.collaterals[0].lif, WAD).mulDivUp(ORACLE_PRICE_SCALE, collateral);
    }
    ...
    function _maxRepaid(uint256 units, uint256 debt, uint256 oraclePrice) internal view returns (uint256) {
        ...
        return (debt - _maxDebt).mulDivUp(WAD, WAD - obligation.collaterals[0].lif.mulDivUp(lltv, WAD));
    }
