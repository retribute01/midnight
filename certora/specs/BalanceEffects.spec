...
/// The continuous fee recipient can't authorize another account, because it can't call
...
strong invariant feeClaimerCantAuthorize(address authorized)
    !isAuthorized(Utils.continuousFeeRecipient(), authorized)
    {
        preserved with (env e) {
            require e.msg.sender != Utils.continuousFeeRecipient(), "continuous fee recipient can't call";
...
strong invariant feeClaimerHasNoPendingFee(bytes32 id)
    pendingFee(id, Utils.continuousFeeRecipient()) == 0
    {
        preserved take(uint256 units, address taker, address takerCallback, bytes takerCallbackData, address receiver, Midnight.Offer offer, bytes ratifierData, bytes32 root, bytes32[] proof) with (env e) {
            require e.msg.sender != Utils.continuousFeeRecipient(), "continuous fee recipient can't call";
            require offer.maker != Utils.continuousFeeRecipient(), "continuous fee recipient can't be maker";
            require taker != Utils.continuousFeeRecipient(), "continuous fee recipient can't be taker";
            requireInvariant feeClaimerCantAuthorize(e.msg.sender);
        }
    }
...
/// Assumes the continuous fee recipient can't call since its address derives from the hash of a human readable string.
rule takeEffects(env e, uint256 units, address taker, address takerCallback, bytes takerCallbackData, address receiver, Midnight.Offer offer, bytes ratifierData, bytes32 root, bytes32[] proof, bytes32 anyId, address anyUser) {
    ...
    require e.msg.sender != continuousFeeRecipient, "continuous fee recipient can't call";
    require offer.maker != continuousFeeRecipient, "continuous fee recipient can't be maker";
    require taker != continuousFeeRecipient, "continuous fee recipient can't be taker";
    requireInvariant feeClaimerCantAuthorize(e.msg.sender);
